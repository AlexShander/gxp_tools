...
sid=$(curl -s -c /tmp/cookies.txt -d"password=$hardpassword" -H "Referer: http://$ip_address/" http://$ip_address/cgi-bin/dologin | grep sid | sed -r 's|.*"sid"[[:space:]]*: "([0-9a-z]+)".*|\1|' )
if [ ! -z $sid ]
then
  echo "OK Login $ip_address"
  changeupgradesettings="P238=0&P212=1&P192=$upgradehttpserver&P194=3&&sid=${sid}"
  ...
  curl -s -X "POST" -b /tmp/cookies.txt -d"request=REBOOT&sid=${sid}" \
    --header "Cache-Control: max-age=0" \
    --header "Origin: http://$ip_address" \
    --header "Referer: http://$ip_address/" \
    --header "ccept-Encoding: gzip, deflate" \
    --header "Accept-Language: ru-RU,ru;q=0.8,en-US;q=0.6,en;q=0.4" \
    --header "User-Agent: Chrome/58.0.3029.110 Safari/537.36" \
    http://$ip_address/cgi-bin/api-sys_operation
else
